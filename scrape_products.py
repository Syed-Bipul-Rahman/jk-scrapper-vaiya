...
def login(session: requests.Session) -> bool:
    ...
    try:
        ...
        # Find the login form; some sites embed it within an <form> tag
        form = soup.find("form")
        if not form:
            print("Warning: No form found on login page; attempting to post credentials directly.")
            # Fallback: send credentials directly to the login URL
            payload = {"Email": EMAIL, "Password": PASSWORD}
            post_resp = session.post(LOGIN_URL, data=payload, timeout=20)
            return post_resp.status_code == 200

        # Extract form action (relative or absolute URL)
        action = form.get("action") or LOGIN_URL
        login_action_url = action if action.startswith("http") else requests.compat.urljoin(LOGIN_URL, action)

        # Prepare payload: include all hidden inputs and the credentials
        ...
        # Fill in our credentials; attempt to guess field names based on common
        # patterns (e.g. "Email", "email", "username", etc.). Update both
        # uppercase and lowercase possibilities.
        for key in list(payload.keys()):
            lower_key = key.lower()
            if "email" in lower_key or "username" in lower_key:
                payload[key] = EMAIL
            elif "pass" in lower_key:
                payload[key] = PASSWORD

        # In case the form did not include expected fields, add them anyway
        if not any(k.lower() in ("email", "username") for k in payload):
            payload["email"] = EMAIL
        if not any("pass" in k.lower() for k in payload):
            payload["password"] = PASSWORD

        print(f"Submitting login form to {login_action_url}…")
        post_resp = session.post(login_action_url, data=payload, timeout=20)
        post_resp.raise_for_status()

        # Check whether login succeeded by looking for a "Logout" link or
        # redirect back to account page.  Some sites return 302 on success.
        if post_resp.history and post_resp.history[0].status_code in (301, 302):
            print("Login redirects detected; assuming success.")
            return True

        # Alternatively, check for keywords in the response
