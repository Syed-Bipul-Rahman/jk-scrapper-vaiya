...
def parse_collection_page(session: requests.Session, collection_url: str) -> List[str]:
    ...
    # The site uses various classes for product cards. Try multiple selectors
    selectors = [
        "div.grid__item",
        "div.product-card",
        "li.grid__item",
    ...
    for sel in selectors:
        ...
        if cards:
            break
    if not cards:
        # Fallback: search for any anchor tags under an element with
        # class `product` or `product-item`
...
def process_category(session: requests.Session, category: Dict[str, str]) -> None:
    ...
    for style in STYLE_CODES:
        ...
        while True:
            url = base_collection
            if page > 1:
                url = f"{base_collection}?page={page}"
            ...
            for link in product_links:
                ...
                # Skip if we've already scraped this product
                # Use the URL path to derive a simple key. Some product
                # URLs may include parameters or trailing slashes; we
                # remove them for consistent lookup.
                ...
                # Only download if not already present (prevents duplicates)
